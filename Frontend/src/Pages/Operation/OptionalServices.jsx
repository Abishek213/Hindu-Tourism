...
const optionalServiceService = {
  ...
  updateServiceStatus: async (id, isActive) => {
    const response = await api.put(`/optService/${id}/status`, { is_active: isActive });
    return response.data;
  },
...
const FormInput = ({ label, name, value, onChange, type = 'text', required = false, options = [], placeholder = '' }) => (
  <div className="mb-4">
...
const ServiceRow = ({ service, index, onEdit, onToggleStatus, onDelete }) => (
  <tr className={index % 2 === 0 ? 'bg-white' : 'bg-orange-25'}>
    ...
    <td className="px-6 py-4 font-semibold text-orange-600">Rs.{service.price}</td>
...
const ServiceFormModal = ({ isEdit, isOpen, formData, onInputChange, onSave, onClose, isLoading }) => {
        ...
        <div>
          ...
          <div className="mb-4">
            <label className="flex items-center">
              ...
              />
              <span className="text-orange-900 font-medium">Active Service</span>
            </label>
          </div>
...
const OptionalServicesDashboard = () => {
  ...
  const fetchServices = async (showRefreshLoader = false) => {
    try {
      ...
      const response = currentView === 'active' 
  ...
  const handleCreateService = async () => {
    ...
    try {
      ...
      toast.error(error.response?.data?.message || error.message || 'Creation failed');
      console.error('Error:', error);
    } finally {
      setIsSaving(false);
    }
  ...
  const handleUpdateService = async () => {
    ...
    try {
      setIsSaving(true);
      const response = await optionalServiceService.updateService(selectedService._id, {
        name: formData.name.trim(),
        description: formData.description.trim(),
        price: parseFloat(formData.price),
        is_active: formData.is_active
  ...
  const handleToggleStatus = async (serviceId, isActive) => {
    try {
      const response = await optionalServiceService.updateServiceStatus(serviceId, isActive);
      if (response.success) {
        ...
        <div className="max-w-7xl mx-auto px-4 py-6">
          <div className="flex justify-between items-center">
            <div className="flex items-center gap-4">
              <h1 className="text-3xl font-bold text-orange-900">Optional Services Management</h1>
              {isRefreshing && (
            ...
            <div className="flex gap-3">
              <select
                ...
                className="px-4 py-2 border border-orange-200 rounded-lg focus:ring-2 focus:ring-orange-500"
              >
          ...
          <div className="overflow-x-auto">
            <table className="w-full">
              <thead className="bg-orange-100">
                <tr>
                  <th className="px-6 py-4 text-left text-orange-900 font-semibold">Service Name</th>
                  <th className="px-6 py-4 text-left text-orange-900 font-semibold">Description</th>
                  ...
                  <tr>
                    <td colSpan="5" className="px-6 py-8 text-center text-gray-500">
                      <div className="flex flex-col items-center">
                        ...
                        <p className="text-sm">Create your first optional service to get started!</p>
                      </div>
                    </td>
                  </tr>
                )}
