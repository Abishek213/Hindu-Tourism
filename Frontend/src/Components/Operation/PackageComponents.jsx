...
export const PackageFormModal = ({ isEdit, isOpen, formData, onInputChange, onSave, onClose }) => {
...
export const ItineraryViewModal = ({ isOpen, selectedPackage, onClose, onAddDay, onEditDay }) => {
      ...
      <div className="bg-white rounded-xl p-6 w-full max-w-4xl mx-4 max-h-[90vh] overflow-y-auto">
        <div className="flex justify-between items-center mb-6">
          <div>
            ...
            <p className="text-orange-700">{selectedPackage.duration} days • {selectedPackage.destinations}</p>
          </div>
...
export const FormInput = ({ label, name, value, onChange, placeholder, type = 'text', small = false }) => (
  <div>
    <label className={`block text-orange-800 font-medium mb-1 ${small ? 'text-xs' : 'text-sm'}`}>
      {label}
    </label>
    <input
      type={type}
      ...
      value={value || ''}
      onChange={onChange}
      className={`w-full ${small ? 'p-2 text-sm' : 'p-2.5'} border border-orange-200 rounded-lg focus:ring-2 focus:ring-orange-500 focus:border-transparent`}
      placeholder={placeholder}
    />
  </div>
);

export const FormTextarea = ({ label, name, value, onChange, placeholder, small = false }) => (
  <div>
    <label className={`block text-orange-800 font-medium mb-1 ${small ? 'text-xs' : 'text-sm'}`}>
      {label}
    </label>
    <textarea
      ...
      value={value || ''}
      onChange={onChange}
      className={`w-full p-2 border border-orange-200 rounded-md focus:ring-1 focus:ring-orange-500 focus:border-transparent ${small ? 'h-12 text-sm' : 'h-20'} resize-none`}
      placeholder={placeholder}
    />
  </div>
);
...
export const PackageRow = ({ pkg, index, onView, onEdit, onToggleStatus }) => (
  <tr className={`border-b ${index % 2 === 0 ? 'bg-orange-25' : 'bg-white'} hover:bg-orange-50 transition-colors`}>
